...
const UserProfile = ({ activeUser, onBack }) => {
  ...
  // console.log("channelProfile : ", activeUser);
  ...
  useEffect(() => {
    ...
    const fetchSubscriptionData = async () => {
      try {
        const { data: subData } = await api.get(`/subscription/getUserSubscription/${activeUser?._id}`);
  ...
  useEffect(() => {
    const fetchUserVideo = async () => {
      ...
      try {
        const { data } = await api.get(`/video/uservideos/c/${activeUser.username}`);
  ...
  useEffect(() => {
    const fetchChannel = async () => {
      try {
        const { data } = await api.get(`/users/c/${activeUser?.username}`);
    ...
    if (activeUser?.username) fetchChannel();
  }, [activeUser?.username]);
          ...
          <div className="flex flex-col md:flex-row md:items-center px-8 pt-4 gap-8">
            ...
            <div className=''>
              <h1 className="text-3xl font-bold">
                {channelProfile?.fullName || activeUser?.fullName || activeUser?.username || 'Unknown User'}
              </h1>
              <div className="text-gray-400">
                @{channelProfile?.username || authUsername || 'unknown'}
              </div>
