...
const UserProfile = ({ username: propUsername }) => {
  ...
  // fetch subscriber count of a user. 
    ...
    const fetchSubscriptionData = async () => {
      try {
        ...
        const { data: subData } = await api.get(`/subscription/getUserSubscription/${authUser?._id}`);
  ...
  useEffect(() => {
    const fetchUserVideo = async () => {
      if (!authUser?.username) return; // Don't fetch if no username

      try {
        const { data } = await api.get(`/video/uservideos/c/${authUser.username}`);
  ...
  useEffect(() => {
    const fetchChannel = async () => {
      try {
        const { data } = await api.get(`/users/c/${profileUsername}`);
    ...
    if (profileUsername) fetchChannel();
  }, [profileUsername]);
      ...
      <div className="flex flex-col md:flex-row md:items-center px-8 pt-4 gap-8">
        ...
        <div className=''>
          <h1 className="text-3xl font-bold">
            {channelProfile?.fullName || authFullname || authUsername || 'Unknown User'}
          </h1>
